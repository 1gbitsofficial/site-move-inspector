...
```powershell
.\scripts\build-wordpress-release.ps1
```

The script validates version alignment and creates a deterministic, production-only ZIP under `dist/wordpress/`. Development dependencies, tests, internal docs, and WordPress.org marketing assets are excluded.

Before submission, also run Plugin Check against an installed copy of the generated ZIP.

## License

GPL-2.0-or-later. See the repository-level [`LICENSE.txt`](../../LICENSE.txt).
